...
def percentsge_pred(request):
    if request.method == 'POST':
        try:
            ...
            return JsonResponse(error_response, status=400)  # 400 Bad Request 에러 응답

    # POST 요청이 아닌 경우 또는 위의 try-except 블록을 통과하지 못한 경우
    return JsonResponse({}, status=405)  # 405 Method Not Allowed 에러 응답
